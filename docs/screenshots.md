...
README中的三张图来自在[TingQue.ai](https://tingque.ai/)实际完成的牌谱解析和段位计算，图片中的百分比对应具体局面。
...
README中的三張圖來自於[TingQue.ai](https://tingque.ai/)實際完成的牌譜解析和段位計算，圖片中的百分比對應具體局面。
...
The three README images were captured after completing replay analysis and a rank calculation on [TingQue.ai](https://tingque.ai/). Percentages in the images refer to particular positions.
...
本次使用的是 2016 年的一局天凤四人东南战牌谱（13个小局）。**此处不登载该牌谱的编号与直链**——截图虽已匿名显示，但一旦给出原局地址，任何人都能把玩家A–D 对回真实账号，而报告中含有针对具体一手的评价。可用自己的牌谱查看相同类型的分析指标；具体数值会随牌谱和模型版本变化。

本次使用的是 2016 年的一局天鳳四人東南戰牌譜（13個小局）。**此處不登載該牌譜的編號與直連**——截圖雖已匿名顯示，但一旦給出原局地址，任何人都能把玩家A–D 對回真實帳號，而報告中含有針對具體一手的評價。可用自己的牌譜查看相同類型的分析指標；具體數值會隨牌譜和模型版本變化。

A 2016 Tenhou four-player East–South replay (13 hands) was used. **Its replay ID and direct link are deliberately omitted here** — the screenshots are anonymised, but publishing the original replay address would let anyone map Player A–D back to real accounts, and the report contains judgements about individual discards. Analyze your own replay to explore the same types of metrics; values will differ by replay and model version.
...
官网段位分析的输入为南3局、0本场、0供托、四家39,000／28,000／18,000／15,000点、特上四段；官网按百点输入为390／280／180／150。点击「分析」后，截图中的四家pt EV依次为+72.0、+29.0、−26.2、−59.7。

官網段位分析的輸入為南3局、0本場、0供託、四家39,000／28,000／18,000／15,000點、特上四段；官網按百點輸入為390／280／180／150。點擊「分析」後，截圖中的四家pt EV依序為+72.0、+29.0、−26.2、−59.7。

The website rank calculation uses South 3, zero honba and deposits, scores of 39,000 / 28,000 / 18,000 / 15,000 and Tokujou / 4-dan. The website expects scores in hundreds: 390 / 280 / 180 / 150. After calculation, its displayed pt EV values are +72.0, +29.0, −26.2 and −59.7.
