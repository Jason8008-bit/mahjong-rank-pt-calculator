...
参与修改时，请同步 Python/TypeScript 两套算法与简体中文、繁体中文、英文三版 README；涉及计分或概率时补充回归案例并运行完整检查。本地分析器截图与官网产品截图的来源、对应范围和更新步骤见[截图说明](docs/screenshots.md)。

參與修改時，請同步 Python/TypeScript 兩套演算法與簡體中文、繁體中文、英文三版 README；涉及計分或機率時補充回歸案例並執行完整檢查。公開範例請使用虛構局面，避免包含真實玩家的私人牌譜或個人資料。

Keep website product screenshots and local application screenshots labeled with their actual source. Follow the [screenshot update instructions](docs/screenshots.md); browser automation updates the local analyzer image only.
