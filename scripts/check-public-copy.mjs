...
const files = [...readmes, 'index.html', 'docs/algorithm.md', 'docs/screenshots.md', 'CONTRIBUTING.md', 'CHANGELOG.md',
...
readmes.forEach((path, index) => {
  ...
  for (const text of [...names[index], ...readmes, 'docs/images/tingque-replay.jpg', 'docs/images/tingque-replay-analysis.jpg',
    'docs/images/tingque-rank-analysis.jpg', 'docs/images/analyzer.png', 'docs/screenshots.md', 'https://tingque.ai/',
    'npm ci', 'npm run dev', 'npm run build', 'npm run build:standalone', 'npm run verify', 'npm run test:browser',
    'rankpt analyze examples/request.json', 'from rankpt import analyze', '1e-12', '160', 'MIT']) {
    if (!content.includes(text)) errors.push(`${path}: missing required information ${text}`);
  }
});
for (const path of [...readmes, 'docs/algorithm.md', 'docs/screenshots.md', 'CONTRIBUTING.md', 'CHANGELOG.md']) {
...
console.log(`Public-copy PASS: ${files.length} files; three complete README editions and relative links checked.`);
